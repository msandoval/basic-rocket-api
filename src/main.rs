...
fn fetch_all_todo_items() -> Result<Json<ToDoList>, String> {
    ...
    let results = statement.query_map([], |row| {
        Ok(ToDoItem{ id: row.get(0)?, item: row.get(1)? })
    });
    ...
    match results {
        Ok(rows) => {
            ...
            match collection {
                Ok(items) => return Ok(Json(ToDoList{items})),
                Err(_) => return Err("Could not collect items".into()),
            };
        },
        Err(_) => return Err("Failed to fetch todo items".into()),
    }
...
fn add_todo_item(item: Json<String>) -> Result<Json<StatusMessage>, String> {
    ...
    match results {
        Ok(rows_affected) => return Ok(Json(StatusMessage{message: format!("{} rows inserted", rows_affected)})),
        Err(_) => return Err("Failed to insert todo item".into()),
    }
...
fn remove_todo_item(id: i64) -> Result<Json<StatusMessage>, String> {
    ...
    match results {
        Ok(rows_affected) => return Ok(Json(StatusMessage{message: format!("{} rows deleted", rows_affected)})),
        Err(_) => return Err("Failed to insert todo item".into()),
    }
...
fn main() {
    ...
    rocket::ignite().mount("/", routes![index,fetch_all_todo_items,add_todo_item,remove_todo_item]).launch();
}